...
def _parse_detail(soup, url):
    ...
    for section in soup.select("section.detail-section"):
        ...
        pairs = _label_value_pairs(section, "div.item", "span.label", "span.value")
        if name and pairs:
            details[name] = pairs
    ...
    detail["rental_notes"] = (
        _label_value_pairs(
            service, "div.desc-item", "span.desc-label", "span.desc-value"
        )
        if service
        else {}
    )
    ...
    detail["poster"] = _text(
        soup.select_one("section.contact p.base-info-pc span.name")
    )
