...
- 可選擇使用 OpenCode Go 或 Zen AI，依完整詳情與照片評估物件，並將推薦結果與理由附在通知中。
- 每個縣市使用獨立 SQLite 資料表。
...
docker run -d \
  ...
  -e OPENCODE_ZEN_API_KEY="你的 OpenCode Zen API key（選填）" \
...
- **OpenCode Go**（預設）：需訂閱 Go 方案，設定 `OPENCODE_GO_API_KEY`。模型如 `kimi-k3`、`glm-5.3` 等。
- **OpenCode Zen**：免費模型可無 API Key 使用（如 `mimo-v2.5-free`、`nemotron-3-ultra-free` 等），付費模型需 `OPENCODE_ZEN_API_KEY`。

在 Telegram 選單的「🤖 AI 評估」啟用功能，可切換提供者與模型。`ai.criteria` 可填入個人偏好，例如「預算兩萬內、重視採光、步行十分鐘內到捷運」。
...
與 message ID。若傳送失效或結果因中斷而無法確認，下一次爬蟲會自動重送。
